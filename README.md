# option-trade-summarizer-skill

提取期权交易记录截图（OCR），通过 Python 脚本进行结构化统计、计算交易成本，并生成多维可视化 HTML 报告。
...
```bash
gemini skills install ./option-trade-summarizer-skill
```
...
当用户提供一张或多张期权交易记录截图，并要求进行归纳、汇总或可视化分析时，请严格按照以下工作流执行：
